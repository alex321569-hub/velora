...
type CheckpointPriority = "critical" | "watch" | "info";
...
function buildCheckpoints(
  ...
  if (opinion.resistance && opinion.resistanceDistance !== null && opinion.resistanceDistance <= 3) {
    add(`${formatValue(opinion.resistance.price, currency)} 저항 돌파`, "추세 지속 여부를 판단하는 핵심 가격대", "critical");
  } else if (opinion.resistance) {
    add(`${formatValue(opinion.resistance.price, currency)} 저항 접근`, "상승 여력이 실제로 열리는지 확인", "watch");
  }
  ...
  if (indicators.macd?.status === "하락 신호") {
    add("MACD 골든크로스 여부", "모멘텀 개선이 동반되는지 확인", "watch");
  } else if (indicators.macd?.status === "상승 신호") {
  ...
  if (opinion.support) {
    add(`${formatValue(opinion.support.price, currency)} 지지 여부`, "하락 시 방어선으로 작동하는지 확인", opinion.supportDistance !== null && opinion.supportDistance <= 3 ? "critical" : "watch");
  }
